...
<div class="breadcrumbs" id="breadcrumbs">
	<ul class="breadcrumb">
		...
		<li class="active"><a href="#">Order管理</a></li>
	</ul>
</div>
...
<div class="page-content">
	...
	<div class="page-header">
		...
		<button class="btn btn-danger" data-toggle="modal"
			data-target="#myModalAdd" id="addButton">新增</button>
		</div>
	</div>
	...
	<!-- 新增弹窗 -->
	<div class="modal fade" id="myModalAdd" tabindex="-2" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="myModalLabel">Order创建</h4>
				</div>
					...
					<div style="padding: 25px 100px 10px;">
						<form id="addFormId">
							...
							<div class="input-group">
								...
								<input type="text" class="form-control" placeholder="必填参数" name="userId" id="userIdAdd" />
							</div>
							<div class="input-group">
								<span class="input-group-addon">employeeId</span>
								<input type="text" class="form-control" placeholder="必填参数" name="employeeId" id="employeeIdAdd" />
							</div>
							<div class="input-group">
								<span class="input-group-addon">productId</span>
								<input type="text" class="form-control" placeholder="必填参数" name="productId" id="productIdAdd" />
							</div>
							<div class="input-group">
								<span class="input-group-addon">amount</span>
								<input type="text" class="form-control" placeholder="必填参数" name="amount" id="amountAdd" />
							</div>
							<div class="input-group">
				...
				<div class="modal-footer">
					<button type="button" id="submitAdd" class="btn btn-primary">确认新增</button>
				</div>
			</div>
		</div>
	</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body" style="padding: 25px 100px 10px;">
					<form id="editFormId">
						...
						<div class="input-group">
							<span class="input-group-addon">userId</span>
							<input type="text" class="form-control" placeholder="必填参数" name="userId" id="userIdEdit" />
						</div>
						<div class="input-group">
							<span class="input-group-addon">employeeId</span>
							<input type="text" class="form-control" placeholder="必填参数" name="employeeId" id="employeeIdEdit" />
						</div>
						<div class="input-group">
							<span class="input-group-addon">productId</span>
							<input type="text" class="form-control" placeholder="必填参数" name="productId" id="productIdEdit" />
						</div>
						<div class="input-group">
							<span class="input-group-addon">amount</span>
							<input type="text" class="form-control" placeholder="必填参数" name="amount" id="amountEdit" />
						</div>
						<div class="input-group">
							<span class="input-group-addon">status</span>
				...
				<div class="modal-footer">
					<button type="button" id="submitEdit" class="btn btn-primary">确认修改</button>
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	(function(jQuery) {
        ...
        $("#submitAdd").bind("click", function(e) {
            ...
            LINK_HTTP.post("/order/save", formData, function(data) {
                if (data != 0) {
                    alert("保存失败！");
        ...
        $("#user_table").find("a[class='blue']").on("click",function(e){
            ...
            LINK_HTTP.get("/order/get/"+targetId,function(result){
                if(result){

                    $("#idEdit").val(result.id);
                    $("#userIdEdit").val(result.userId);
                    $("#employeeIdEdit").val(result.employeeId);
		...
		$("#user_table").find("a[class='red']").bind("click", function(e) {
			...
			if (confirm("你确定删除：" + targetId)) {
				if (confirm("再次确定删除：" + targetId)) {
					//提交修改用户信息
					LINK_HTTP.get("/order/delete/"+targetId, function(data) {
						if (data.code == 0) {
							LINK_HTTP.load(".main-content","/order/index");//刷新页面
						}
					}, "json");
				}
			}
